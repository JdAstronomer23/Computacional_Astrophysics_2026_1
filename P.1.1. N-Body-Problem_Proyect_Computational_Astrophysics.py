...
def simulate_restricted_three_body():
    ...
    # --- 3. EJECUCIÓN DEL SOLVER (RK4) ---
    solver = RKSolver()
    for i in range(n_steps - 1):
        states[i+1] = solver.step(deriv_three_body, states[i], times[i], dt, masses, G)
    
    # --- 4. EXTRACCIÓN Y TRANSFORMACIÓN AL MARCO ROTANTE ---
    ...
    # Gráfica A: Marco Inercial (Como se veía antes)
    ...
    # Gráfica B: Marco Rotante (Recomendación técnica)
...
def error_vs_step_size_analysis():
    """
    Analiza cómo el paso temporal h afecta el error global.
    Compara con la fórmula teórica de error de truncación (Ec. 0.6 o 0.7).
    """
    # Sistema simple: oscilador armónico o problema de 2 cuerpos con paso variable
    # Usamos el problema de 2 cuerpos de la Tarea 1 para medir error final vs h
    a_km = 6771.0
    e = 0.0002
    mu = 3.986004418e5
    T = 2 * np.pi * np.sqrt(a_km**3 / mu)
    total_time = T  # un período
    dt_values = [1, 5, 10, 30, 60, 120, 300]  # segundos
    errors = []
    
    for dt in dt_values:
        n_steps = int(total_time / dt)
        # Estado inicial igual que en two_body
        r0 = a_km * (1 - e)
        v0 = np.sqrt(mu * (2/r0 - 1/a_km))
        ...
        for _ in range(n_steps):
            ...
            t += dt
        # Posición final analítica
        pos_kepler_final = keplerian_motion(total_time, a_km, e, mu)
        error_pos = np.linalg.norm(state[0:3] - pos_kepler_final)
        errors.append(error_pos)
    
    # Ajuste de ley de potencias: error ∝ h^4 para RK4 (teórico)
    ...
    plt.loglog(dt_arr, np.exp(coeffs[1]) * dt_arr**coeffs[0], 'r--',
               label=f'Pendiente {coeffs[0]:.2f} (teórico 4)')
    plt.xlabel('Paso temporal h (s)')
    plt.ylabel('Error de posición final (km)')
    plt.title('Análisis de error global vs paso temporal (RK4)')
    plt.legend()
    plt.grid(True)
    plt.savefig('error_vs_step.png', dpi=300)
    plt.show()
    print(f"Análisis de error: pendiente observada = {coeffs[0]:.2f} (esperado 4 para RK4)")
